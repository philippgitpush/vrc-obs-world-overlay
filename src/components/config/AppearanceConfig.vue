  ...
  <form id="config-form" class="p-4">
    ...
    <div>
      ...
      <div class="mt-4 grid xl:grid-cols-6 lg:grid-cols-5 md:grid-cols-4 grid-cols-2 gap-4">
        ...
        :key="placement">
          <div
            class="w-full aspect-[16/9] rounded-xl border-2 cursor-pointer transition-colors hover:border-blue-500"
            :class="{
              'bg-blue-200 border-blue-500': placement === overlay_placement,
              'bg-gray-200 border-gray-300': placement !== overlay_placement
            }"
            @click="setOverlayPlacement(placement)">
            <PlacementTopLeftIcon v-if="placement === 'top-left'" :fill="placement === overlay_placement ? '#3b82f6' : '#d1d5db'" />
            <PlacementTopRightIcon v-if="placement === 'top-right'" :fill="placement === overlay_placement ? '#3b82f6' : '#d1d5db'" />
            <PlacementBottomLeftIcon v-if="placement === 'bottom-left'" :fill="placement === overlay_placement ? '#3b82f6' : '#d1d5db'" />
            <PlacementBottomRightIcon v-if="placement === 'bottom-right'" :fill="placement === overlay_placement ? '#3b82f6' : '#d1d5db'" />
          </div>
